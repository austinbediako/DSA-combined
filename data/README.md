...
| `algorithm_runs.csv` (91 records) | Not researched or synthetic -- extracted directly from the team's own real performance-experiment measurements in `evidence/benchmarks/` (one representative run per algorithm/input-size combination across all 6 required experiments). | No personal data. Programmatically validated: sequential ids, no duplicates, all numeric fields parse correctly, no missing required fields. Confirmed to load into the live SQLite schema. Loaded automatically by the app on first run against an empty database. |

This table is now complete for all 5 CSV-backed entities (locations,
